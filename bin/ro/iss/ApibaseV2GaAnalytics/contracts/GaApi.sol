...
contract GaApi {
    ...
    event GaCustomBatchEvent(string _batchJsonBody);
    event GaEvent(string _hitPayload);
    ...
    mapping (address => address) private memberClientIdMap; // memberAddress => clientIdAddress (clientId is the calling address of the call owner (msg.sender))

    // requestId counter; must be checked not to overflow
    ...
    function gaPost(string _hitPayload) public onlyMember {
        emit GaEvent(_hitPayload);
    }
